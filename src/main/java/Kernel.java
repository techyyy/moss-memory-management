...
public class Kernel extends Thread {
    ...
    public void step() {
        ...
        if (instruct.inst.startsWith("READ")) {
            ...
            page.counter = ++PageFault.counter;

            //System.out.println( Virtual2Physical.pageNum( instruct.addr , virtPageNum , block )+" " +  PageFault.counter);
            if (page.physical == -1) {
                if (doFileLog) {
                ...
                if (doFileLog) {
                    printLogFile("READ " + Long.toString(instruct.addr, addressradix) + " " + page.id + " ... okay");
                }
                if (doStdoutLog) {
